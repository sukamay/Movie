window.onload = function () {
    ...
    //     }
    // };
    let stars = document.querySelectorAll(".star-level");
    for (let i = 0; i < 5; i++){
        stars[i].textContent = (5 - i).toString() + "星"
    }
    let star_lines = document.querySelectorAll(".percent");
    let star_powers = document.querySelectorAll(".power");
    for (let i = 0; i < 5; i++){
        star_powers[i].style.width = parseInt(star_lines[i].textContent) + "px";
    }
    let pra = document.getElementById("main");
    let star = pra.querySelector("#stars");
    let star_score = pra.querySelector(".avg-review-left").textContent;
    star.style.backgroundPosition = "0px " + Math.floor(10 - parseFloat(star_score)) * (-15) + "px";
};
